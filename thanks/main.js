...
document
  .querySelector(".lang-switcher .current")
...
window.addEventListener("click", (event) => {
  if (
    !document.querySelector(".lang-switcher").contains(event.target) &&
...
document
  .querySelectorAll(".main-menu-wrapper > ul > .menu-item-has-children > a")
  .forEach((el) => {
    el.addEventListener("click", (event) => {
      event.preventDefault();
      if (window.innerWidth < mobileBreakpoint) {
...
document
  .querySelectorAll(".main-menu-wrapper > ul > .menu-item-has-children")
  .forEach((el) => {
    ...
    el.addEventListener("mouseleave", (event) => {
      if (window.innerWidth >= mobileBreakpoint) {
        ...
        document
          .querySelectorAll(
            ".main-menu-wrapper .mega-menu-item > .sub-menu > .menu-item"
          )
          .forEach((el) => {
            el.classList.remove("active");
            $(el).find(".sub-menu").css("min-height", "");
...
document
  .querySelectorAll(
    ".main-menu-wrapper .mega-menu-item > .sub-menu > .menu-item > a"
  )
  .forEach((el) => {
    el.addEventListener("mouseenter", (event) => {
      if (window.innerWidth >= mobileBreakpoint) {
...
document.querySelectorAll("#popup .close-btn, #overlay").forEach((el) =>
  el.addEventListener("click", (event) => {
    event.preventDefault();
    fadeOut(overlay);
  ...
  pagination: {
    ...
    renderBullet: function (index, className) {
      ...
      return (
        ...
        '"><span>' +
        index +
        '</span><span class="pagination-sq"><span></span></span></div>'
      );
    },
  },
  autoplay: {
...
function OpenSocial() {
  ...
  if (social.classList.contains("listSocial--active")) {
    ...
    document.querySelector(".openSocial-sft").innerHTML =
      '<i class="icon-close-icon" style="font-size: 25px;"></i>';
  }
}
